...
if [ -x "$PROJECT_DIR/bin/diff-quality" ]; then
  EVIDENCE="Run \`bin/diff-quality --no-browser --no-tests\` to check for rubycritic regressions against the base branch. For a deeper pass that includes tests and coverage, the user can run /simplify-with-analysis explicitly."
else
